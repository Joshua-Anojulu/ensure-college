...
class TestReminders:
    def _saved_with_deadline(self, client, days_out):
        """Sign up and save a scholarship whose deadline is `days_out` days away, if any."""
        from datetime import date, timedelta
        target = (date.today() + timedelta(days=days_out)).isoformat()
        scholarships = client.get("/scholarships").json()
        match = next((s for s in scholarships if s["deadline"] == target), None)
        return match

    def test_me_exposes_reminder_pref_default_on(self, client):
        signup(client)
        assert client.get("/auth/me").json()["reminders_enabled"] is True

    def test_toggle_reminders(self, client):
        signup(client)
        off = client.patch("/account/reminders", json={"enabled": False})
        assert off.status_code == 200 and off.json()["enabled"] is False
        assert client.get("/auth/me").json()["reminders_enabled"] is False
        on = client.patch("/account/reminders", json={"enabled": True})
        assert on.json()["enabled"] is True

    def test_reminders_toggle_requires_login(self, client):
        assert client.patch("/account/reminders", json={"enabled": False}).status_code == 401

    def test_unsubscribe_link_disables_reminders(self, client):
        from app.db.database import SessionLocal
        from app.db.models import User

        signup(client, email="unsub@example.com")
        db = SessionLocal()
        try:
            token = db.query(User).filter(User.email == "unsub@example.com").first().reminder_unsubscribe_token
        finally:
            db.close()
        assert token
    ...
    def test_run_endpoint_requires_cron_secret(self, client, monkeypatch):
        ...
        assert client.get("/reminders/run").status_code == 404
    ...
    def test_run_endpoint_disabled_without_secret(self, client, monkeypatch):
        ...
        assert client.get("/reminders/run", headers={"Authorization": "Bearer anything"}).status_code == 404


class TestReminderComputation:
    def test_due_items_window_and_email(self):
        from datetime import date, timedelta

        from app.reminders import DueItem, build_reminder_email

        today = date.today()
        items = [
            DueItem("Gates Scholarship", "Scholarship", today + timedelta(days=3), 3, "https://x.org"),
            DueItem("Rec letter — Ms. Rivera", "Recommendation letter", today + timedelta(days=1), 1, None),
        ]
        subject, text, html = build_reminder_email(items, "tok123")
        assert "2 saved opportunities closing soon" == subject
        assert "Gates Scholarship" in text and "Gates Scholarship" in html
